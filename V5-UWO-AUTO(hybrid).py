...
actions = [
    ...
    {"voltage": -12, "time": 1500},  # Move backward to scoring zone
...
def autonomous():
    ...
    for attempt in range(3): 
        for action in actions:
            ...
            drive_left.stop(brakeType.BRAKE)
            drive_right.stop(brakeType.BRAKE)
            
            # brakeType::brake : The motor actively resists movement. 
            # This is more abrupt than coasting and is useful when you want the robot to stop more quickly
            # but not hold its position against external forces.
            
            
            # arm.spin(direction, % of the motor's maximum speed, units in which speed is specified)

            
            
            # Special action for picking up Triball
            if action == actions[0]:  # If this is the action to reach the Triball
                arm.spin(directionType.FWD, 50, velocityUnits.PCT)  # Pick up Triball
                wait(1000, MSEC)  # Adjust based on mechanism's speed and timing
                # brakeType.HOLD : Specifies that once the arm motor is stopped, it should hold its position firmly. 
                # This is useful for preventing the Triball from falling out if the arm is holding it off the ground.
                arm.stop(brakeType.HOLD)
            
            # Special action for depositing Triball
            if action == actions[1]:  # If this is the action to deposit the Triball
                arm.spin(directionType.REV, 50, velocityUnits.PCT)  # Deposit Triball
